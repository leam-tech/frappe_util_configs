...
io.on('connection', async function (socket) {
  // [frappe_util_configs] We have many cases when host and origin doesnt match
	// if (get_hostname(socket.request.headers.host) != get_hostname(socket.request.headers.origin)) {
	// 	return;
	// }

  // [frappe_util_configs] We have many cases when there are no cookies
	...
	// [frappe_util_configs] getRenovationUserDetails obsoletes this
...
function getRenovationUserDetails(socket) {
  return new Promise((resolve, rej) => {
    ...
    request.get(get_url(socket, '/api/method/frappe_util_configs.realtime.get_user_info'))
		.set(headers)
    .type('form')
    .query(query)
    .end(function (err, res) {
